...
import ChangeOutstandings from './ChangeOutstandings'
import { Company } from '@/types'

const ChangeOutstandingsModal = ({ closeModal, company }: { closeModal: () => void; company: Company }) => {
  return (
    <div className='fixed inset-0 bg-black bg-opacity-60 z-50 transition duration-300 flex justify-center items-start overflow-x-hidden overflow-y-auto'>
      <div className='bg-white dark:bg-bgDark relative drop-shadow-md rounded-lg p-10 my-1.5'>
        ...
        <ChangeOutstandings closeModal={closeModal} company={company} />
      </div>
    </div>
  )
}

export default ChangeOutstandingsModal
