...
const Orders = ({ orders }: { orders: Order[] }) => {
  ...
  return (
    <div className='ml-5 md:flex justify-center gap-10'>
      <div>
          ...
          orders.map((order) => {
            return (
              <div
                key={order.id}
                className={`${
                  orderInfo.ticket === order.ticket && 'border-b-title'
                } border-b-2 flex w-fit pt-2 m-2 cursor-pointer`}
                onClick={() => setOrderInfo(order)}
              >
                <BookmarkIcon
                  className={`${
                    orderInfo.ticket === order.ticket && 'text-title fill-current'
                  } w-6`}
                />
      ...
      {orderInfo && (
        <div className='mt-10'>
          <section>
            <h2 className='text-gray-600'>Estado:</h2>
            <h2 className='font-bold ml-5'>{translateStatus[orderInfo.status]}</h2>
          </section>
          <section>
            <h2 className='text-gray-600'>Restaurante:</h2>
